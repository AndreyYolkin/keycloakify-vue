...
(async () => {
    ...
    const starterNames = fs
      ...
      .filter(
        (basename) =>
          ...
          basename.includes('vue') &&
          fs.statSync(pathJoin(parentDirPath, basename)).isDirectory(),
      );

    if (starterNames.length === 0) {
      console.log(chalk.red(`No starter found. Keycloakify Vue starter found in ${parentDirPath}`));
      process.exit(-1);
    }
  ...
  {
    const child = child_process.spawn('npx', ['tsx', pathJoin(getThisCodebaseRootDirPath(), 'scripts', 'watch.ts')], {
      shell: true,
