...
<template>
    <div v-if="isGroupLabel" :class="kcClsx('kcFormGroupClass')" v-bind="html5DataAnnotations">
        <div :class="kcClsx('kcContentWrapperClass')">
            <label :id="`header-${attribute.group?.name}`" :class="kcClsx('kcFormGroupHeader')">
                <component v-if="groupDisplayHeader !== ''" :is="i18n.advancedMsg(groupDisplayHeader)" />
                <template v-else>{{ attribute.group?.name ?? "" }}</template>
            </label>
        </div>
        <div v-if="groupDisplayDescription !== ''" :class="kcClsx('kcLabelWrapperClass')">
            <label :id="`description-${attribute.group?.name}`" :class="kcClsx('kcLabelClass')">
