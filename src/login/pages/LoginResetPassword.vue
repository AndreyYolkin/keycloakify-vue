...
<template>
    ...
    >
        ...
        <template #info>
            <component v-if="realm.duplicateEmailsAllowed" :is="i18n.msg('emailInstructionUsername')" />
            <component v-else :is="i18n.msg('emailInstruction')" />
        </template>

        <form id="kc-reset-password-form" :class="kcClsx('kcFormClass')" :action="url.loginAction" method="post">
            <div :class="kcClsx('kcFormGroupClass')">
                <div :class="kcClsx('kcLabelWrapperClass')">
                    <label for="username" :class="kcClsx('kcLabelClass')">
                        <component v-if="!realm.loginWithEmailAllowed" :is="i18n.msg('username')" />
                        <component v-else-if="!realm.registrationEmailAsUsername" :is="i18n.msg('usernameOrEmail')" />
